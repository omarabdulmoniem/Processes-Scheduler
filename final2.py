...
def fcfsplot():
    p = GraphClass(int(num0.get()))
    for i in range(int(num0.get())):
        p.add_bar(processes[i].id,processes[i].burstTime)
    p.plot_graph()
...
def fcfs_window():
    ...
    global seq_label1
    seq_label1 = Label(new,text='')

    global fcnum_label
    fcnum_label = Label(new,text='')
            ...
            if len(readyQueue) != 0:
                ...
                if processData[k][2]>=1:
                  ...
                  if processData[k][2] == 0:                 #if burst time is zero, it means process is completed
                    ...
                    processData[k].append(eTime-(1-processData[k][2]))            #give no 6 for the completion time
                    processData[k][2]=0
                    processData[k][4] = 1                                        #mark the process as completed
            ...
            if len(readyQueue) == 0:
                ...
                if processData[k][2]>=1:
                   ...
                   if processData[k][2] == 0:                 #if burst time is zero, it means process is completed
                    ...
                    processData[k].append(eTime-(1-processData[k][2]))            #give no 6 for the completion time
                    processData[k][2]=0
                    processData[k][4] = 1                                        #mark the process as completed     #give no 6 for the completion time
...
def prioritypre_window():
    ...
    global seq_label
    seq_label = Label(new,text='')

    global ppnum_label
    ppnum_label = Label(new,text='')
...
class PnonGraphClass:
    ...
    def plot_graph(self):
        ...
        titlee = ''
        for i in range(int(num0.get())):
            titlee += '   ' + 'P' + str(i+1) + ': ' + str(self.colors_list[i+1]) 
        plt.title(titlee)
        plt.show()
...
def Pnon_plot():
    p = PnonGraphClass(int(num0.get()))
    for i in range(int(num0.get())):
        index = int(sq[i])-1
        bur = burlist[index]
        p.add_bar(int(sq[i]),bur)
    p.plot_graph()
...
class Priority_non_preemptive:
    ...
    def print_data(self, processData, av_waiting_time, av_turn_around_time, process_seq):
        ...
        seqpnon_label.config(text = ff , bg= '#772020',fg='white',font=('Arial', 14))
...
def prioritynon_window():
    ...
    global seqpnon_label
    seqpnon_label = Label(new,text='')

    global pnonnum_label
    pnonnum_label = Label(new,text='')
            ...
            if len(ready_queue) != 0:
                ...
                for k in range(len(process_data)):
                    if process_data[k][0] == ready_queue[0][0]:
                        break
                if process_data[k][2]>=1:
                  process_data[k][2] = process_data[k][2] - 1  #remove 1 from the burst time 
                  if process_data[k][2] == 0:                 #if burst time is zero, it means process is completed
                    process_data[k][3] = 1                   #mark the process as completed
                    process_data[k].append(e_time)            #give no 6 for the completion time
                else:
                    process_data[k].append(e_time-(1-process_data[k][2]))            #give no 6 for the completion time
                    process_data[k][2]=0
                    process_data[k][3] = 1                                        #mark the process as completed
            ...
            if len(ready_queue) == 0:
                ...
                if process_data[k][2]>=1:
                   ...
                   if process_data[k][2] == 0:                 #if burst time is zero, it means process is completed
                    ...
                    process_data[k].append(e_time-(1-process_data[k][2]))            #give no 6 for the completion time
                    process_data[k][2]=0
                    process_data[k][3] = 1                                        #mark the process as completed     #give no 6 for the completion time
    ...
    def printData(self, process_data, average_turnaround_time, average_waiting_time, sequence_of_process):
        ...
        for i in range(len(sequence_of_process)):
            if i != len(sequence_of_process)-1 and sequence_of_process[i] == sequence_of_process[i+1]:
                continue
            ff += '  ' + 'P' + str(int(sequence_of_process[i]+1)) + '  |'
...
def sjfpre_window():
    ...
    global seqsjf_label
    seqsjf_label = Label(new,text='')

    global psjfnum_label
    psjfnum_label = Label(new,text='')
...
def sjfnon_window():
    ...
    global seqsjfnon_label
    seqsjfnon_label = Label(new,text='')

    global sjfnum_label
    sjfnum_label = Label(new,text='')
...
def rr_window():
    ...
    global sqq_label
    sqq_label = Label(new,text='')

    global sqqnum_label
    sqqnum_label = Label(new,text='')
